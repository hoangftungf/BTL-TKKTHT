...
class UnifiedKGClient:
    ...
    def add_product(
        ...
        Stores `name_lower` on Product, Category, and Brand nodes
        to enable index-backed CONTAINS queries (avoiding toLower() in WHERE).
        """
    ...
    def search_products(
        ...
        Uses `name_lower` fields + pre-lowered params to leverage indexes.
        toLower() is only used as fallback for description (no index needed).
        """
        session = self._get_session()
        if not session:
            return []
        ...
        if query:
            where_clauses.append(
                "(p.name_lower CONTAINS $query_lower OR toLower(p.description) CONTAINS $query_lower)"
            )
            params["query_lower"] = query.lower()

        if category:
            where_clauses.append("c.name_lower CONTAINS $category_lower")
            params["category_lower"] = category.lower()

        if brand:
            where_clauses.append("b.name_lower CONTAINS $brand_lower")
            params["brand_lower"] = brand.lower()
    ...
    def create_indexes(self):
        """Create recommended indexes — optimized for common query patterns.

        Includes:
        - Basic indexes on id fields
        - name_lower indexes for index-backed CONTAINS queries (Phase 6)
        - Composite index for common filter pattern (status + price)
        """
        session = self._get_session()
        if not session:
            return
        indexes = [
            ...
            # name_lower indexes (avoid toLower() in WHERE — enables index usage)
            "CREATE INDEX product_name_lower IF NOT EXISTS FOR (p:Product) ON (p.name_lower)",
            "CREATE INDEX category_name_lower IF NOT EXISTS FOR (c:Category) ON (c.name_lower)",
            "CREATE INDEX brand_name_lower IF NOT EXISTS FOR (b:Brand) ON (b.name_lower)",
            ...
            # Composite index: status + price (common filter pattern)
